...
std::ostream& operator<< (std::ostream& os, const Hotkey& hotkey)
{
    ...
    if (hotkey.fsModifiers_ & MOD_CONTROL) {
        os << " Ctrl";
    }
    if (hotkey.fsModifiers_ & MOD_ALT){
        os << " Alt";
    }
    if (hotkey.fsModifiers_ & MOD_WIN){
        os << " Win";
    }
    if (hotkey.vk_ != VK_SHIFT && hotkey.vk_ != VK_CONTROL
        && hotkey.vk_ != VK_MENU && hotkey.vk_ != VK_LWIN
        && hotkey.vk_ != VK_RWIN){
        os << " " << vkToString(hotkey.vk_);
    }
    return os;
}
...
public:
    ...
    LRESULT OnKeydown(UINT,  WPARAM wParam, LPARAM , BOOL& bHandled){
        ...
        if (GetKeyState(VK_SHIFT) < 0){
            hk_.fsModifiers_ |= MOD_SHIFT;
        }
        if (GetKeyState(VK_CONTROL) < 0){
            hk_.fsModifiers_ |= MOD_CONTROL;
        }
        if (GetKeyState(VK_MENU) < 0){
            hk_.fsModifiers_ |= MOD_ALT;
        }
        if (GetKeyState(VK_LWIN) < 0 || GetKeyState(VK_RWIN) < 0){
            hk_.fsModifiers_ |= MOD_WIN;
        }
        std::ostringstream owss;
        owss << hk_;
        ::SetWindowTextA(m_hWnd, owss.str().c_str());
        LOG(logINFO) << owss.str();
        return TRUE;
    }
